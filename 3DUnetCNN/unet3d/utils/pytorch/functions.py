...
# Computing Precision 
def precision(y, predicted_y):
        true_positives = keras.sum(keras.round(keras.clip(y * predicted_y, 0, 1)))
        predicted_positives = keras.sum(keras.round(keras.clip(predicted_y, 0, 1)))
        precision = true_positives / (predicted_positives + keras.epsilon())
        return precision


# Computing Sensitivity      
def sensitivity(y, predicted_y):
    true_positives = keras.sum(keras.round(keras.clip(y * predicted_y, 0, 1)))
    possible_positives = keras.sum(keras.round(keras.clip(y, 0, 1)))
    return true_positives / (possible_positives + keras.epsilon())


# Computing Specificity
def specificity(y, predicted_y):
    true_negatives = keras.sum(keras.round(keras.clip((1-y) * (1-predicted_y), 0, 1)))
    possible_negatives = keras.sum(keras.round(keras.clip(1-y, 0, 1)))
